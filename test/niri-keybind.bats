...
@test "niri-keybind list emits valid JSON array" {
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" list
    [ "$status" -eq 0 ]
...
@test "niri-keybind validate accepts valid key" {
    run python3 "$NIRI_KEYBIND" validate "Mod+T"
    [ "$status" -eq 0 ]
}

@test "niri-keybind validate rejects empty string" {
    run python3 "$NIRI_KEYBIND" validate ""
    [ "$status" -eq 4 ]
}
...
@test "niri-keybind set rebinds key atomically" {
    ...
    cp "$NIRI_CONFIG" "$bak"
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" set "Mod+T" "Mod+RETURN"
    [ "$status" -eq 0 ]
...
@test "niri-keybind set preserves indent" {
    local bak="$NIRI_CONFIG.bak"
    cp "$NIRI_CONFIG" "$bak"
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" set "Mod+SPACE" "Mod+D"
    [ "$status" -eq 0 ]
...
@test "niri-keybind set no-ops on same key" {
    local bak="$NIRI_CONFIG.bak"
    cp "$NIRI_CONFIG" "$bak"
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" set "Mod+T" "Mod+T"
    [ "$status" -eq 0 ]
...
@test "niri-keybind set reports conflict for already-bound key" {
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" set "Mod+T" "Mod+Q"
    [ "$status" -eq 2 ]
}

@test "niri-keybind set reports not-found for missing key" {
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" set "Mod+Z" "Mod+X"
    [ "$status" -eq 3 ]
}

@test "niri-keybind get returns JSON for existing key" {
    run python3 "$NIRI_KEYBIND" --config "$NIRI_CONFIG" get "Mod+T"
    [ "$status" -eq 0 ]
    echo "$output" | jq -e '.key == "Mod+T"' >/dev/null
}

@test "niri-keybind selftest passes" {
    run python3 "$NIRI_KEYBIND" selftest
    [ "$status" -eq 0 ]
